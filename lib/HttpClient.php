...
class HttpClient
{
    ...
    public function _parse_and_convert_to_api_response($resp_content, $status_code, $method, $request_params, $request_data)
    {
        ...
        throw new \MagicAdmin\Exception\ApiException(
            '',
            \property_exists('status', $resp_content) ? $resp_content->status : null,
            $status_code,
            \property_exists('data', $resp_content) ? $resp_content->data : null,
            \property_exists('message', $resp_content) ? $resp_content->message : null,
            \property_exists('error_code', $resp_content) ? $resp_content->error_code : null,
            $request_params,
            $request_data,
            $method
        );
