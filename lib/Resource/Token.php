...
class Token {
    ...
    public function get_public_address($did_token) {
        return \MagicAdmin\Util\parse_public_address_from_issuer($this->get_issuer($did_token));
    }
    ...
    public function validate($did_token) {
        ...
        $current_time_in_s = \MagicAdmin\Util\epoch_time_now();
        ...
        if ($current_time_in_s < \MagicAdmin\Util\apply_did_token_nbf_grace_period($claim->nbf)) { 
            throw new \MagicAdmin\Exception\DIDTokenException(
